...
const files = (await readdir('data/nsw', { withFileTypes: true }))
  .filter((file) => file.isFile() && file.name.endsWith('.json'))
  .sort();
const tenders = JSON.parse(await readFile('data/tenders.json', 'utf8')) as Tender[];
const known = new Set(
  tenders.filter((tender) => tender.source === 'nsw').map((tender) => tender.id),
);
let added = 0;
...
if (added) {
  ...
  await writeFile('data/tenders.json.tmp', output);
  await rename('data/tenders.json.tmp', 'data/tenders.json');
}
