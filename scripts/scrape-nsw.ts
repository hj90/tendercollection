...
if (!prcOpportunityId || !/^[A-Za-z0-9-]+$/.test(prcOpportunityId))
  throw new Error('Usage: npm run scrape:nsw -- <prcOpportunityId>');
...
const response = await fetch('https://api.anakin.io/v1/url-scraper', {
  ...
  headers,
  body: JSON.stringify({
    url: `https://buy.nsw.gov.au/prcOpportunity/${prcOpportunityId}`,
    country: 'au',
    formats: ['json'],
    generateJson: true,
  }),
...
if (!response.ok)
  throw new Error(`Anakin submit failed: HTTP ${response.status} ${await response.text()}`);
...
if (!result || result.status !== 'completed')
  throw new Error(`Anakin scrape did not complete: ${JSON.stringify(result)}`);

const generatedJson = result.generatedJson;
if (!generatedJson || typeof generatedJson !== 'object' || !('data' in generatedJson))
  throw new Error('Anakin completed without generatedJson.data');
...
if (!data || typeof data !== 'object' || Array.isArray(data))
  throw new Error('Anakin generatedJson.data is not a JSON object');
