...
const read = async <T>(name: string): Promise<T> =>
  JSON.parse(await readFile(`data/${name}.json`, 'utf8'));
const [active, archive, buyers, meta] = await Promise.all([
  read<Tender[]>('tenders'),
  read<ArchivedTender[]>('archive'),
  read<Buyers>('buyers'),
  read<Metadata>('meta'),
]);
const urls: Record<'vendorpanel' | 'austender', string> = {
...
await Promise.all(
  (Object.keys(urls) as ('vendorpanel' | 'austender')[]).map(async (source) => {
    ...
    try {
      const records = await (async () => {
        const xml =
          source === 'austender'
            ? await getAusTenderText(urls[source], warn)
            : await (async () => {
                const response = await fetch(urls[source], {
                  headers: {
                    'User-Agent':
                      'TenderCollection/1.0 (+https://tendercollection.vercel.app/about/; public RSS aggregator)',
        ...
        return source === 'vendorpanel'
          ? await enrichVendorPanel(
              parseVendorPanel(xml, buyers, warn),
              active.filter((t) => t.source === 'vendorpanel'),
              undefined,
              warn,
            )
          : await enrichAusTender(
              parseAusTender(xml, warn),
              (url) => getAusTenderText(url, warn),
              warn,
            );
      })();
      if (new Set(records.map(key)).size !== records.length)
        throw new Error('Duplicate feed identifiers');
      ...
      if (source === 'vendorpanel') {
        const mapped = records.filter((t) => {
          const info = buyerInfo(t.buyer, t.closingAtRaw, buyers, () => {});
          return info.mapped && info.state !== null;
        }).length;
        console.log(
          `VendorPanel buyer-map state coverage: ${mapped}/${records.length} (${((100 * mapped) / records.length).toFixed(1)}%)`,
        );
        if (mapped / records.length < 0.9)
          warn('Buyer-map state coverage below 90%; extend data/buyers.json');
      }
      ...
      const current = active
        ...
        .map(({ firstSeenAt, changes, ...t }) => t)
        .sort((a, b) => key(a).localeCompare(key(b)));
      const sorted = [...records].sort((a, b) => key(a).localeCompare(key(b)));
      if (JSON.stringify(current) !== JSON.stringify(sorted) || meta[source].status !== 'ok')
        meta[source] = { status: 'ok', snapshotAt: now, message: null };
      ...
      warn(`${message}; retaining previous snapshot.`);
      meta[source] = {
        ...meta[source],
        status: 'unavailable',
        message:
          'Feed unavailable; previous snapshot retained. Check the source for current information.',
      };
      report.sources[source] = {
        status: 'unavailable',
        error: message,
        lastSuccessfulSnapshotAt: meta[source].snapshotAt,
        warnings,
      };
    }
  }),
...
if (!Object.keys(snapshots).length) {
  console.error('All feeds unavailable; snapshot retained.');
  process.exitCode = 1;
}
