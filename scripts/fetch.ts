...
await Promise.all((Object.keys(urls) as Source[]).map(async source=>{
 ...
 const records=source==='vendorpanel'?parseVendorPanel(xml,buyers,warn):parseAusTender(xml,warn);
 ...
 const message=error instanceof Error?error.message:String(error);warn(`${message}; retaining previous snapshot. No HTML fallback.`);
