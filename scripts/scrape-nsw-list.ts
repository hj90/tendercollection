...
async function scrapePage(page: number): Promise<NSWPage> {
  ...
  if (!submit.ok)
    throw new Error(
      `Anakin submit failed for page ${page}: HTTP ${submit.status} ${await submit.text()}`,
    );
  ...
  for (let attempt = 0; attempt < 60; attempt++) {
    const poll = await fetch(
      `https://api.anakin.io/v1/url-scraper/${encodeURIComponent(submitted.jobId)}`,
      { headers },
    );
    if (!poll.ok)
      throw new Error(
        `Anakin poll failed for page ${page}: HTTP ${poll.status} ${await poll.text()}`,
      );
  ...
  if (!result || result.status !== 'completed')
    throw new Error(`Anakin scrape failed for page ${page}: ${JSON.stringify(result)}`);
  const data = result.generatedJson?.data;
  if (!data || typeof data !== 'object' || Array.isArray(data))
    throw new Error(`Anakin page ${page} has no generatedJson.data object`);
  return data as NSWPage;
}

console.log('Processing page 1');
const first = await scrapePage(1);
if (!Array.isArray(first.jobs) || typeof first.totalJobs !== 'number')
  throw new Error('NSW page 1 did not contain jobs and totalJobs');
const jobs: NSWJob[] = [];
...
for (let page = 2; page <= lastPage && jobs.length < totalJobs; page++) {
  ...
  if (jobs.length === before) {
    console.warn(`NSW pagination stopped making progress at page ${page}`);
    break;
  }
  await save();
}
if (jobs.length !== totalJobs)
  console.warn(`NSW pagination ended with ${jobs.length} of ${totalJobs} jobs`);
