...
export default function Listing({items,index,history=false,detailItems}:{items:ListingType[];index:SearchIndex;history?:boolean;detailItems?:Tender[]}){
 const params=useSearchParams();const router=useRouter();const pathname=usePathname();const [now,setNow]=useState<number|null>(null);const [limit,setLimit]=useState(30);const [draft,setDraft]=useState(params.get('q')??'');const [view,setView]=useState<'list'|'split'>('list');const [selected,setSelected]=useState<string|null>(null);
 ...
 const card=(t:ListingType)=>{const key=itemKey(t);const selectedCard=view==='split'&&key===selected;return <article className={'tender-card '+(t.requestType==='forward-notice'?'forward ':'')+(selectedCard?'selected-tender':'')} key={key}>
  ...
  <div className="card-title-row"><h2><Link href={tenderPath(t)} onClick={event=>{if(view==='split'&&window.matchMedia('(min-width: 901px)').matches){event.preventDefault();setSelected(key)}}}>{t.title}</Link></h2><a className="source-link" href={sourceUrl(t)} target="_blank" rel="noopener noreferrer">View source ↗</a></div>
 ...
 <div className="filter-row">{select('state','States',options('state'),{unknown:'State not specified'})}{select('category','Categories',categoryGroups.map(([key])=>key),categoryLabels)}{!history&&select('buyer','Buyers',options('buyer'))}{select('type','Request types',Object.keys(typeLabels),typeLabels)}{select('source','Sources',['vendorpanel','austender'],sourceLabels)}
