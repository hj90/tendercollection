...
export function classify(title: string, description: string): RequestType {
  const forward =
    /\b(?:forward notice|advance[d]? (?:tender )?notice|prior information notice|future tender|intention to (?:go to market|invite)|do not (?:respond|submit)|no (?:need to provide a response|response (?:is )?required))\b/i;
  if (forward.test(title + ' ' + description)) return 'forward-notice';
  const rules: [RequestType, RegExp][] = [
    [
      'panel',
      /\b(?:prequalified|pre-qualified|standing offer|preferred supplier|supplier (?:panel|register)|panel arrangement)\b/i,
    ],
    ['eoi', /\b(?:eoi|expressions? of interest)\b/i],
    ['rfq', /\b(?:rfq|requests? for quot(?:e|es|ation|ations))\b/i],
    ['rfp', /\b(?:rfp|requests? for proposals?)\b/i],
    ['rft', /\b(?:rft|requests? for tenders?|invitation to tender)\b/i],
  ];
...
export function parseDate(
  ...
  if (raw) {
    const m = raw.match(
      /(\d{1,2}\/\w{3}\/\d{4}\s+\d{1,2}:\d{2}\s*[AP]M)\s*\(UTC([+-]\d{2}:\d{2})\)/i,
    );
    const date = m
      ? DateTime.fromFormat(`${m[1]} ${m[2]}`, 'd/LLL/yyyy h:mm a ZZ', {
          locale: 'en',
          setZone: true,
        })
...
export function buyerInfo(
  ...
  const match =
    buyers[buyer] ??
    Object.entries(buyers).find(([name]) => normaliseName(name) === normaliseName(buyer))?.[1];
  if (match) return { state: match.state, buyerType: match.type, mapped: true };
  warn(`Buyer map miss: ${buyer}`);
  const labels: Record<string, State> = {
    Brisbane: 'QLD',
    Adelaide: 'SA',
    Perth: 'WA',
    Hobart: 'TAS',
...
export function safeSourceUrl(raw: string, source: 'vendorpanel' | 'austender' | 'nsw') {
  ...
  const domain = source === 'vendorpanel' ? 'vendorpanel.com.au' : 'tenders.gov.au';
