...
export function closedNoticeRedirect(original: string, location: string): string {
  const source = new URL(noticeUrl(original));
  const destination = new URL(location, source);
  const expected = source.pathname.replace('/Show/', '/ShowClosed/');
  if (
    destination.origin !== source.origin ||
    destination.pathname !== expected ||
    destination.hash ||
    ![...destination.searchParams].every(([k, v]) => k === 'PreviewMode' && v === 'False')
  )
    throw new Error('Unexpected AusTender redirect; snapshot retained');
  return destination.href;
}
export async function getAusTenderText(url: string, warn: (s: string) => void): Promise<string> {
  if (url !== FEED) noticeUrl(url);
  const headers = {
    'User-Agent': AUSTENDER_USER_AGENT,
    Accept: url === FEED ? 'application/rss+xml, application/xml, text/xml' : 'text/html',
  };
  ...
  if ([301, 302, 303, 307, 308].includes(response.status)) {
    ...
    if (!location) throw new Error('Missing notice redirect location');
    ...
    response = await fetch(closedUrl, {
      headers,
      redirect: 'error',
      signal: AbortSignal.timeout(30000),
    });
