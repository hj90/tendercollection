...
export function parseNSW(data: NSWData, warn: (message: string) => void): ParsedTender[] {
  ...
  const records = data.jobs.map((job, index) => {
    if (!job.title || !job.company || !job.url)
      throw new Error(`NSW job ${index} is missing required fields`);
    const sourceUrl = safeSourceUrl(job.url, 'nsw');
    const publishedAt = job.postedDate
      ? parseDate(
          DateTime.fromFormat(job.postedDate, 'd-LLL-yyyy HH:mm', {
            locale: 'en',
            zone: 'Australia/Sydney',
          }).toISO() ?? job.postedDate,
          warn,
          `NSW job ${job.url}`,
        )
      : null;
    return {
      id: new URL(sourceUrl).pathname.split('/').filter(Boolean).pop() ?? sourceUrl,
      source: 'nsw' as const,
      reference: null,
      title: job.title,
  ...
  if (new Set(records.map((record) => record.id)).size !== records.length)
    throw new Error('Duplicate NSW opportunity identifiers');
  return records;
}

export function parseNSWDetail(
  data: NSWDetail,
  sourceUrl: string,
  ...
  const date = (raw: string | null, context: string) => {
    if (!raw) return null;
    const parsed = DateTime.fromFormat(raw, 'dd-LLL-yyyy h:mm a', {
      locale: 'en',
      zone: 'Australia/Sydney',
    });
    if (parsed.isValid) return parsed.toUTC().toISO();
    warn(`${context}: unparseable date ${JSON.stringify(raw)}`);
    return null;
  };
  const publishedAt = metadata.publishDate
    ? date(metadata.publishDate + ' 12:00 PM', `NSW detail ${source} publish date`)
    : null;
  const documents = (data.links ?? [])
    .map((link) => link.url?.trim().replace(/^https:\/{2,}/i, 'https://'))
    .filter((url): url is string =>
      Boolean(url && /^https?:\/\//i.test(url) && /\.pdf(?:[?#]|$)/i.test(url)),
    );
  const contactName = metadata.primaryContact ?? metadata.primaryContactName;
  const contactParts = [
    contactName,
    metadata.primaryContactEmail,
  ...
  return {
    ...
    requestType: requestType({
      title,
      description,
      jobType: metadata.type ?? null,
      company: buyer,
      location: null,
      postedDate: null,
      url: source,
    }),
    closingAt: date(metadata.closeDate ?? null, `NSW detail ${source} close date`),
    closingAtRaw: metadata.closeDate ?? null,
    publishedAt,
    contact: contactName
      ? {
          name: contactName,
          ...(metadata.primaryContactEmail ? { email: metadata.primaryContactEmail } : {}),
          ...(metadata.primaryContactPhone ? { phone: metadata.primaryContactPhone } : {}),
