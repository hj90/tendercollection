...
import {parseAusTender,parseAusTenderDetail,parseAusTenderClosing,enrichAusTender,noticeUrl} from '../lib/parsers/austender';
...
test('real sparse RSS preserves URL GUID and extracts reference',()=>{assert.equal(seeds.length,73);assert.equal(seeds[0].reference,'PROC-8302');assert.equal(seeds[0].id,seeds[0].sourceUrl);assert.ok(!('buyer' in seeds[0]));});
test('real detail enriches buyer, closing date, contact and complete category',()=>{const t=parseAusTenderDetail(seeds[0],html,()=>{});assert.equal(t.buyer,'Services Australia');assert.equal(t.closingAt,'2026-09-08T04:00:00.000Z');assert.equal(t.contact?.email,'tenders@ndis.gov.au');assert.deepEqual(t.categories,['81112200 - Software maintenance and support']);assert.equal(t.state,null);assert.equal(t.requestType,'rft');});
test('advertisement template works with the same labelled fields',()=>{const seed=seeds.find(t=>t.reference==='PROC-9177452')!;const t=parseAusTenderDetail(seed,fixture('austender-advert.html'),()=>{});assert.equal(t.buyer,'Grains Research and Development Corporation');assert.equal(t.closingAt,'2026-09-10T07:00:00.000Z');});
