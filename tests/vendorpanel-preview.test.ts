...
test('public preview maps complete description, richer dates, requirements, Q&A and updates',()=>{const d=parseVendorPanelPreview(html);assert.equal(d.opportunityDescription,'Complete description\nincluding the final line.');assert.equal(d.buyerReference,'ABC-42');assert.equal(d.opensAt,'2025-04-02T14:00:00.000Z');assert.equal(d.queryCutoffAt,'2027-06-30T04:00:00.000Z');assert.equal(d.background,'Background detail');assert.equal(d.desiredOutcomes,'Outcome\ndetail');assert.deepEqual(d.buyerQuestions,['Can you deliver?']);assert.deepEqual(d.serviceRegions,['Queensland\nCentral West']);assert.equal(d.publicQuestions[0].question,'Can we edit the form?');assert.equal(d.publicQuestions[0].answer,'Yes, you may.');assert.equal(d.updates[0].description,'A new schedule was attached.');assert.equal(parseVendorPanelPreviewDate('26/Aug/2026 04:33 PM'),'2026-08-26T06:33:00.000Z')});

test('preview enrichment uses the full description and retains prior details on failure',async()=>{const seed={id:'bfbbade7954f4db693dca121e7e7b7d3',source:'vendorpanel',reference:'VP455349',title:'Example',description:'Brief...',buyer:'Buyer',buyerType:null,state:null,categories:[],requestType:'other',closingAt:null,closingAtRaw:null,publishedAt:null,contact:null,documentCount:null,sourceUrl:'https://www.vendorpanel.com.au/PublicTenderAccess.aspx?id=x'} satisfies ParsedTender;const parsed=parseVendorPanelPreview(html);const {opportunityDescription,...details}=parsed;const [enriched]=await enrichVendorPanel([seed],[],async()=>html);assert.equal(enriched.description,opportunityDescription);const prior={...seed,firstSeenAt:'2026-01-01T00:00:00Z',changes:[],vendorPanelDetails:details} satisfies Tender;const warnings:string[]=[];const [result]=await enrichVendorPanel([seed],[prior],async()=>{throw new Error('offline')},m=>warnings.push(m));assert.deepEqual(result.vendorPanelDetails,details);assert.match(warnings[0],/retained previous details/);assert.match(vendorPanelPreviewUrl(seed),/bfbbade.*s455349$/)});
